...
export default async function Page({params} : props) {
  ...
  const scheduleId = typeof id === 'string' ? parseInt(id, 10) : null;
  ...
  const titleSubText = nominationStatus == 'RSVP Confirmed' ? 'RSVP: You Are Registered' : nominationStatus == 'RSVP Denied' ? 'RSVP: You Have Declined' : 'You Have Not Registered';
  const rsvp =  nominationStatus == 'RSVP Confirmed' ? 'Yes' : nominationStatus == 'RSVP Denied' ? 'No' : null;
  ...
  return (
    <article className="flex flex-col gap-2 px-[2.2rem] pb-[4rem] pt-[2rem] text-sos-primary-blue lg:gap-6 lg:px-[14rem] lg:pb-[8rem] lg:pt-[6rem]">
      ...
      <br />
      <div className="w-96 mb-4"><SectionSeparator /></div>
      
      <RSVPSelector userId={userId?.value} scheduleId={scheduleId} rsvp={rsvp} />
      <div className="text-justify text-12 text-sos-primary-blue lg:text-16">
        <h3 className="mb-4 text-[#B42222]">1. Course Attendance Requirement</h3>
        <p>To ensure successful completion, all courses at the Service Leadership Institue require full attendance:</p>
        <ul className="list-inside list-disc indent-4 m-4">
          <li>
          ...
          <li>
            ...
            <ul className="list-inside list-disc indent-8">
              ...
              <li>More than one absense results in an "imcomplete" mark.</li>
      ...
      <div className="text-justify text-12 text-sos-primary-blue lg:text-16">
        <h3 className="mb-4 text-[#B42222]">2. Course Completion Requirement</h3>
        <ul className="list-inside list-disc indent-4 m-4">
          <li>
           Course Reflection: After completing the course, you must fill out an dsubmit the assigned course reflection form available on the course website.
          </li>
          <br/>
          <li>
            Evaluation Process: Your submitted reflection will be reviewed by the course facilitator. Approval of your reflection is required to receive credit for the course.
          </li>
          <br/>
          <li>
            Granting of credit: Course credit will only be awarded upon successful completion of both attendance and the approved course reflection.
          </li>
        </ul>
        <p>Failure to meet these requirements may result in no credit being granted for the course. If you have any question about these terms, please contact the course facilitatr before proceeding with your registration.</p>
      </div>
    </article>
  );
}
