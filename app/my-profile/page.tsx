...
const Info = ({profileInfo, children}: {profileInfo: ProfileInfo, 
  children: ReactNode;}) => {
    return (
        ...
        />
          <div className="text-16 text-sos-primary-blue lg:text-20 text-justify px-10 lg:px-20">{children}</div>
        </div>
      );
}
...
const  ToDos = () => {
    return (
        <article className="flex flex-col gap-2 px-[2.2rem] pb-[4rem] pt-[2rem] text-sos-secondary-blue lg:gap-6 lg:px-[14rem] lg:pb-[8rem] lg:pt-[6rem]">
            <h1 className="self-start text-center text-32 font-medium lg:text-38">
                To Do's
            </h1>
        </article>
...
async function getServerSideProps(userId: string | undefined): Promise<ProfileInfo> {
  const res = await fetch(
    'https://scd.sos.org/api/CommonServicesController/getSevadarIDFromDnnUserID?dnnUserID='+userId,
    {
      cache: 'no-store',
      method: 'GET',
      headers: {
